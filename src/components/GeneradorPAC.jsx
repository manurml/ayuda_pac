...
const CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .pac-header { text-align: center; margin-bottom: 36px; }
  .pac-header h1 {
    font-family: 'Fraunces', serif;
    font-size: 2.1rem; font-weight: 600;
    color: var(--verde); line-height: 1.15;
  }
  .pac-header h1 em { font-style: italic; color: var(--tierra); }
  .pac-header p { color: var(--gris); font-size: 0.92rem; margin-top: 6px; font-weight: 300; }
  ...
  .step-circle {
    ...
    border: 2px solid #ccc; background: white; color: #aaa;
    transition: all .3s;
  }
  .step-circle.active { background: var(--verde); border-color: var(--verde); color: white; }
  .step-circle.done { background: var(--verde3); border-color: var(--verde3); color: white; }
  .step-label { font-size: 0.7rem; color: var(--gris); font-weight: 400; text-align: center; max-width: 72px; line-height: 1.2; }
  .step-label.active { color: var(--verde); font-weight: 500; }
  .step-line { width: 60px; height: 2px; background: #ddd; margin-bottom: 20px; transition: background .3s; }
  .step-line.done { background: var(--verde3); }

  .card {
    background: white; border-radius: var(--r);
    border: 1px solid #e2ddd4; padding: 32px 28px;
    max-width: 680px; margin: 0 auto 20px;
    box-shadow: 0 2px 12px rgba(45,80,22,.06);
  }
  .card-title { font-family: 'Fraunces', serif; font-size: 1.25rem; font-weight: 600; color: var(--verde); margin-bottom: 4px; }
  .card-sub { font-size: 0.82rem; color: var(--gris); margin-bottom: 24px; font-weight: 300; }
  ...
  label { font-size: 0.78rem; font-weight: 500; color: var(--verde); letter-spacing: .02em; text-transform: uppercase; }
  input, select, textarea {
    font-family: 'DM Sans', sans-serif; font-size: 0.93rem;
    padding: 10px 12px; border: 1.5px solid #ddd; border-radius: 7px;
    background: var(--crema); color: #333; transition: border-color .2s; outline: none; width: 100%;
  }
  input:focus, select:focus, textarea:focus { border-color: var(--verde2); background: white; }
  .err { border-color: var(--error) !important; }
  .err-msg { font-size: 0.74rem; color: var(--error); }

  .parcela-card { background: var(--crema); border: 1px solid #ddd; border-radius: 8px; padding: 16px; margin-bottom: 12px; }
  .parcela-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; }
  .parcela-num { font-size: 0.78rem; font-weight: 500; color: var(--verde); text-transform: uppercase; letter-spacing: .04em; }
  .btn-remove { background: none; border: none; cursor: pointer; color: var(--error); font-size: 1.1rem; padding: 2px 6px; border-radius: 4px; line-height: 1; }
  .btn-remove:hover { background: #fde8e8; }
  .btn-add {
    display: flex; align-items: center; gap: 6px;
    font-family: 'DM Sans', sans-serif; font-size: 0.85rem; font-weight: 500;
    color: var(--verde2); background: none; border: 1.5px dashed var(--verde3);
    border-radius: 8px; padding: 10px 16px; cursor: pointer; width: 100%;
    justify-content: center; transition: background .2s; margin-top: 4px;
  }
  ...
  .ayuda-item {
    ...
    transition: border-color .2s, background .2s; margin-bottom: 10px; background: var(--crema);
  }
  .ayuda-item.selected { border-color: var(--verde2); background: #f2f9ea; }
  .ayuda-item input[type=checkbox] { width: 18px; min-width: 18px; height: 18px; accent-color: var(--verde2); margin-top: 1px; cursor: pointer; }
  ...
  .resumen-label { font-size: 0.72rem; font-weight: 500; color: var(--gris); text-transform: uppercase; letter-spacing: .05em; margin-bottom: 8px; padding-bottom: 4px; border-bottom: 1px solid #e2ddd4; }
  .resumen-row { display: flex; justify-content: space-between; font-size: 0.87rem; padding: 5px 0; }
  .resumen-row span:first-child { color: var(--gris); font-weight: 300; }
  .resumen-row span:last-child { font-weight: 500; color: var(--verde); }
  .resumen-parcela { background: var(--crema); border-radius: 6px; padding: 10px 12px; margin-bottom: 8px; font-size: 0.84rem; }
  .badge { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 0.75rem; font-weight: 500; background: #e8f4d9; color: var(--verde2); margin-right: 6px; margin-bottom: 4px; }

  .nav-btns { display: flex; gap: 12px; justify-content: flex-end; max-width: 680px; margin: 0 auto; }
  .btn { font-family: 'DM Sans', sans-serif; font-size: 0.9rem; font-weight: 500; padding: 11px 24px; border-radius: 8px; cursor: pointer; border: none; transition: all .2s; }
  .btn-secondary { background: white; border: 1.5px solid #ccc; color: var(--gris); }
  ...
  .btn-export { background: var(--tierra); color: white; }
  .btn-export:hover { background: #b07828; }

  .success-wrap { text-align: center; padding: 20px; }
  .success-icon { font-size: 3rem; margin-bottom: 16px; }
  .success-wrap h2 { font-family: 'Fraunces', serif; font-size: 1.5rem; color: var(--verde); margin-bottom: 8px; }
  .success-wrap p { color: var(--gris); font-size: 0.9rem; font-weight: 300; margin-bottom: 24px; }
`;
...
// ─── Subcomponentes FUERA del componente principal (fix pérdida de foco) ──────

function StepTitular({ titular, setT, errTitular }) {
  return (
    <div className="card">
...
function StepParcelas({ parcelas, setP, removeParcela, addParcela, errParcelas }) {
  return (
    <div className="card">
      ...
      {parcelas.map((p, i) => (
        <div key={p.id} className="parcela-card">
          <div className="parcela-header">
            ...
            {parcelas.length > 1 && (
              <button className="btn-remove" onClick={() => removeParcela(i)}>✕</button>
            )}
          </div>
          <div className="form-grid">
            <div className="field">
...
function StepAyudas({ ayudas, toggleAyuda, errAyudas }) {
  return (
    <div className="card">
...
function StepResumen({ titular, parcelas, ayudas }) {
...
export default function GeneradorPAC() {
  ...
  const [titular, setTitular] = useState({
    ...
    municipio: "", provincia: "", cp: "", num_explotacion: "", banco_iban: ""
  });
...
<style>
  ...
  .header-logo{display:flex;justify-content:space-between;border-bottom:2px solid #2D5016;padding-bottom:10px;margin-bottom:16px}
  .org h2{font-size:13px;color:#2D5016;margin:0 0 2px}.org p{font-size:10px;color:#666;margin:0}
  .doc-ref{text-align:right;font-size:10px;color:#666}.doc-ref strong{font-size:12px;color:#2D5016;display:block}
...
<div class="header-logo">
  <div class="org"><h2>MINISTERIO DE AGRICULTURA, PESCA Y ALIMENTACIÓN</h2>
  <p>Fondo Español de Garantía Agraria (FEGA)</p>
  <p>Solicitud única de ayudas — Campaña ${new Date().getFullYear()}</p></div>
  <div class="doc-ref"><strong>SOLICITUD ÚNICA PAC</strong>
  <span>Ref: PAC-${new Date().getFullYear()}-${titular.nif?.replace(/\D/g,"").slice(0,6)}</span><br/>
  <span>Fecha: ${fecha}</span></div>
</div>
<h1>Solicitud Única de Ayudas Directas PAC — Campaña ${new Date().getFullYear()}</h1>
<div class="aviso">⚠ Este documento es un pre-formulario. Debe presentarse oficialmente a través del sistema SIGPAC de su comunidad autónoma o en la oficina comarcal agraria.</div>
...
<table>
  ...
  ${titular.banco_iban?`<tr><td><strong>IBAN</strong></td><td colspan="3">${titular.banco_iban}</td></tr>`:""}
</table>
<div class="section-title">2. RELACIÓN DE PARCELAS DECLARADAS</div>
<table>
  <tr><th>Provincia</th><th>Municipio</th><th>Polígono</th><th>Parcela</th><th>Recinto</th><th>Cultivo</th><th>Variedad</th><th>Superficie (ha)</th></tr>
  ${parcelas.map(p=>`<tr><td>${p.provincia}</td><td>${p.municipio}</td><td>${p.poligono}</td><td>${p.parcela}</td><td>${p.recinto}</td><td>${p.cultivo}</td><td>${p.variedad||"—"}</td><td style="text-align:right">${parseFloat(p.superficie).toFixed(4)}</td></tr>`).join("")}
  <tr style="font-weight:bold;background:#e8f4d9"><td colspan="7">SUPERFICIE TOTAL DECLARADA</td><td style="text-align:right">${superficieTotal} ha</td></tr>
</table>
...
<p style="font-size:10px;line-height:1.6;margin-bottom:10px">El solicitante declara bajo su responsabilidad que los datos consignados son verídicos y que cumple los requisitos establecidos en la normativa vigente para la concesión de las ayudas solicitadas.</p>
<div class="firma">
  <div class="firma-box"><p>Firma del titular</p><p>${titular.nombre}</p><p>D.N.I.: ${titular.nif?.toUpperCase()}</p></div>
  <div class="firma-box"><p>Sello y firma de la oficina comarcal</p><p>Fecha: ___/___/_____</p><p>N.º registro: ________________</p></div>
</div>
</body></html>`;
    ...
    return (
      <>
        ...
        <div className="pac-wrap">
          <div className="pac-header"><h1>Solicitud <em>PAC</em></h1></div>
          <div className="card">
            <div className="success-wrap">
              <div className="success-icon">🫒</div>
              <h2>¡Solicitud lista para exportar!</h2>
              <p>Haz clic en el botón para generar el documento.<br />
                Imprímelo y preséntalo en tu oficina comarcal agraria.</p>
              <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
                <button className="btn btn-export" onClick={exportarPDF}>📄 Exportar / Imprimir documento</button>
                <button className="btn btn-secondary" onClick={() => { setStep(0); setDone(false); }}>Nueva solicitud</button>
              </div>
            </div>
          </div>
        </div>
    ...
    <>
      ...
      <div className="pac-wrap">
        <div className="pac-header">
          ...
          <p>Rellena los datos y exporta el formulario — sin gestoría</p>
        </div>
        ...
        {step === 0 && <StepTitular titular={titular} setT={setT} errTitular={errTitular} />}
        {step === 1 && <StepParcelas parcelas={parcelas} setP={setP} removeParcela={removeParcela} addParcela={addParcela} errParcelas={errParcelas} />}
        {step === 2 && <StepAyudas ayudas={ayudas} toggleAyuda={toggleAyuda} errAyudas={errAyudas} />}
        {step === 3 && <StepResumen titular={titular} parcelas={parcelas} ayudas={ayudas} />}

        <div className="nav-btns">
          {step > 0 && <button className="btn btn-secondary" onClick={goBack}>← Anterior</button>}
